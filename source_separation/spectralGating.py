...
# s1, samplerate = librosa.load(str( audio_path+ file_name + '.wav'))
# s2, samplerate2 = librosa.load(str( drums_path+ file_name + '.wav'))
samplerate, s1 = read(str( audio_path+ file_name + '.wav'))
samplerate2, s2 = read(str( drums_path+ file_name + '.wav'))

reduced_noise = nr.reduce_noise(y = s1, y_noise = s2, sr = samplerate)


#print(s2.shape, 'SHAPE')
...
# yg1 = removeNoise(audio_clip=s1, noise_clip=s2,
#     n_grad_freq=2,
#     n_grad_time=4,
#     n_fft=2048,
#     win_length=2048,
...
# #wf.write(str(output_folder  +file_name +'_s1_predicted_ICA2.wav'), int(samplerate),recovered_sources[:,0].astype(np.float32))