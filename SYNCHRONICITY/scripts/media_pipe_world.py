...
mp_pose = mp.solutions.pose
mp_drawing = mp.solutions.drawing_utils
pose = mp_pose.Pose(  min_detection_confidence= min_conf,
               min_tracking_confidence=min_track_conf, model_complexity = model_comp)
...
while cap.isOpened():
    ...
    # if result.pose_world_landmarks: # use this if you want to have world_landmarks instead
    if result.pose_world_landmarks:
        mp_drawing.draw_landmarks(
            frame, result.pose_world_landmarks, mp_pose.POSE_CONNECTIONS)
        ...
        mp_drawing.draw_landmarks(
            black_frame, result.pose_world_landmarks, mp_pose.POSE_CONNECTIONS)
...
df.to_csv( output_path + date + "_pose_world_data_"+ file_name + ".csv", index=True)
print("Data saved to "+ output_path + date +"_pose_world_data_"+ file_name + ".csv")
...
with open(str(output_path + date + "_pose_world_data_"+ file_name +  ".json"), "w") as outfile:
