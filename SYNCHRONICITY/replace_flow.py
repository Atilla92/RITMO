...
class Flow:
    ...
    def dense(
        ...
        if velocity:
            ...
            # y velocity 
            time_y = np.linspace(0, len(yvel)/fps, len(yvel))
            acceleration_y = self.get_acceleration(yvel, fps)
            acceleration_y_entropy = entropy(acceleration_y)
            ...
            data = {
                ...
                "yvel": yvel,
                "dt_v": acceleration,
                "dt_v_entropy": acceleration_entropy,
                "dt_v_avg": round(np.mean(acceleration),3),
                "time": time,
                'time_y': time_y,
                'acceleration_y': acceleration_y,
                'acceleration_y_entropy': acceleration_y_entropy,
            }
