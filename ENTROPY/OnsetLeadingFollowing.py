...
input_drums = '/Users/atillajv/CODE/RITMO/ENTROPY/output/main/24_Aug_2023_all_Onset_drums_1s/'
#input_ensemble = '/Users/atillajv/CODE/RITMO/ENTROPY/output/main/22_Aug_2023_all_Onset/'
input_guitar = '/Users/atillajv/CODE/RITMO/ENTROPY/output/main/24_Aug_2023_all_Onset_guitar_zd_1s/'

output_path = '/Users/atillajv/CODE/RITMO/ENTROPY/output/main/24_Aug_2023/'
ouput_name = 'LeadingFollowing_1s_'

# Set estimation parameters
leading_ID = 'Guitarist' # Set to "Dancer" if Dancer is leading, set to "Guitarist"
estimate_df_on = False # If you want to estimate 1st ODF, with data['LZ'] 
loop_on = True
name_file = 'P7_D5_G1_M6_R1_T1.csv' # Name for file is loop is off
n_lags = 10
...
averages_dict = {}
print('Number of files analysed:',len(name_files))
...
for i, name_file in enumerate(name_files):
    ...
    try:
        ...
        if estimate_df_on:
            ...
            LZ_dt_guitar = data_guitar['dt_LZ'].to_numpy()
            LZ_dt_drums = data_drums['dt_LZ'].to_numpy()
        ...
        time_lags = range(0, n_lags)  # Lag values from 0 to 10
        ...
        for lag in time_lags:
            ...
            if leading_ID == 'Dancer':
                shifted_series2 = np.roll(LZ_dt_guitar, -lag)
                # Calculate the binary output based on your conditions
                binary_output = np.where((np.sign(LZ_dt_drums) == np.sign(shifted_series2)), 1, 0)
            
            if leading_ID == 'Guitarist':
...
averages_df.to_csv(output_path + ouput_name + leading_ID + '.csv' )
