...
file_name = 'P13_D6_G5_M6_R2_T1' # Name file if loop_on = False
...
file_output = '/Users/atillajv/CODE/RITMO/ENTROPY/output/main/22_Sep_2023_niels/' #check that this is the same as input file 
# Default settings 
percentage = 0.1
...
def loopThroughFiles (list_files, artist_label, percentage = percentage , frac_round = frac_round, dt_L = dt_L, error_files = [], store_i= 0 , df_store = df_store  ):
    ...
    for file_i, file_item in enumerate(list_files):
        ...
        try:
            ...
            #dfE = pd.read_csv('/Users/atillajv/CODE/RITMO/ENTROPY/output/main/22_Sep_2023_all_Onset_1s/'+file_item+'.csv')[1:]
            ...
            scaler_dt = MinMaxScaler(feature_range=(-1,1))

            scaler = MinMaxScaler(feature_range=(0, 1))
            columns_to_normalize = [col for col in p_dfE.columns if col != 'dt_LZ']
            norm_p_dfE = pd.DataFrame(columns = p_dfE.columns)
            norm_p_dfE[columns_to_normalize] = scaler.fit_transform(p_dfE[columns_to_normalize])
            norm_p_dfE['dt_LZ'] = scaler_dt.fit_transform(p_dfE[['dt_LZ']])
            norm_p_dfE = norm_p_dfE.rename(columns={'t0': 'norm_t0'})
            norm_p_dfE['t0'] = p_dfE['t0']


            columns_to_normalize = [col for col in g_dfE.columns if col != 'dt_LZ']
            norm_g_dfE = pd.DataFrame(columns = g_dfE.columns)
            norm_g_dfE[columns_to_normalize] = scaler.fit_transform(g_dfE[columns_to_normalize])
            norm_g_dfE['dt_LZ'] = scaler_dt.fit_transform(g_dfE[['dt_LZ']])
            norm_g_dfE = norm_g_dfE.rename(columns={'t0': 'norm_t0'})
            norm_g_dfE['t0'] = g_dfE['t0']


            # norm_g_data = scaler.fit_transform(g_dfE)
            # norm_g_dfE = pd.DataFrame(norm_g_data, columns = g_dfE.columns)
            # norm_g_dfE = norm_g_dfE.rename(columns={'t0': 'norm_t0'})
            # norm_g_dfE['t0'] = g_dfE['t0']
        ...
        # Arrays for MIR values
            ...
            for i, item in enumerate(tI_0):
                ...
                norm_p_list_Entropy = norm_p_dfE.loc[(norm_p_dfE["t0"] >= tI_0.iloc[i] ) & (norm_p_dfE['t0'] <= tI_1.iloc[i]  ) ]
                ...
                norm_g_list_Entropy = norm_g_dfE.loc[(norm_g_dfE["t0"] >= tI_0.iloc[i] ) & (norm_g_dfE['t0'] <= tI_1.iloc[i]  ) ]
                ...
                df_store.loc[store_i] = {
                    ...
                    'p_ncounts': p_list_Entropy['nCounts'].sum(),
                    'p_ncounts_avg': p_dfE['nCounts'].sum(),
                    'norm_p_dt_LZ' : norm_p_list_Entropy['dt_LZ'].mean(),
                    'norm_p_LZ' : norm_p_list_Entropy['LZ'].mean(),
                    "norm_p_LZ_avg": norm_p_dfE["LZ"].mean(),
                    "norm_p_dt_LZ_avg": norm_p_dfE["dt_LZ"].mean(),
                    'norm_p_IOI': norm_p_list_Entropy['IOI'].mean(),
                    "norm_p_IOI_avg": norm_p_dfE["IOI"].mean(),
                    'norm_p_ncounts': norm_p_list_Entropy['nCounts'].mean(),
                    'norm_p_ncounts_avg': norm_p_dfE['nCounts'].mean(),
                    ...
                    'g_ncounts': g_list_Entropy['nCounts'].sum(),
                    'g_ncounts_tot': g_list_Entropy['nCounts'].sum(),
                    'norm_g_dt_LZ' : norm_g_list_Entropy['dt_LZ'].mean() ,
                    'norm_g_LZ' : norm_g_list_Entropy['LZ'].mean(),
                    "norm_g_LZ_avg" : norm_g_dfE["LZ"].mean(),
                    "norm_g_dt_LZ_avg" : norm_g_dfE["dt_LZ"].mean(),
                    'norm_g_IOI': norm_g_list_Entropy['IOI'].mean(),
                    "norm_g_IOI_avg": norm_g_dfE["IOI"].mean(),    
                    'norm_g_ncounts': norm_g_list_Entropy['nCounts'].mean(),
                    'norm_g_ncounts_avg': norm_g_dfE['nCounts'].mean(),                
...
df_store = df_store.dropna(subset=['Baile_Level'])
print(df_store)
