## This script creates a dataframe the includes all of the the different estimates for each of the ELAN sections. 
## Need to think, because you have dancer ratings and guitarist ratings. Need to process both. And add seperately. 

## Need to run energy for guitarist, and clip the corrupted parts. 
...
file_name = 'P7_P7_D1_G1_M1_R1_T1' # Name file if loop_on = False
...
#file_output = '/Users/atillajv/CODE/RITMO/ENTROPY/output/main/Test/' #check that this 
file_output = '/Users/atillajv/CODE/RITMO/ENTROPY/output/main/all_experiments_07072023_095/' #check that this is the same as 
...
name_output = '14042025_all_experiments_drums_guitar_zd_energy'
# Default settings 
percentage = 0.1
...
list_files = []
error_files = []
body_models = ['lower', 'center', 'upper', 'wholebody']
body_model = body_models[3]
...
print(len(list_files), 'before')
# Remove duplicates from list_files while preserving order
list_files = list(dict.fromkeys(list_files))

print(len(list_files), 'after')
print(list_files)
#list_files = ['G6_P11_D6_G6_M6_R1_T1','P11_P11_D6_G6_M6_R1_T1']

# list_files = ['G5_P10_D1_G5_M1_R1_T1','P10_P10_D1_G5_M1_R1_T1','G5_P10_D1_G5_M1_R2_T1','P10_P10_D1_G5_M1_R2_T1']
...
for file_i, file_item_long in enumerate(list_files):
    ...
    try:
        ...
        # Energy file wholebody dancer
        input_path = '/Users/atillajv/CODE/RITMO/SYNCHRONICITY/scripts/energy_toiviainen_adapted/output/28032025_dancer/'
        participant = file_item.split('_')[0]
        p_df_energy_wholebody = pd.read_csv(input_path + 'Energy_wholebody_'+participant+'_'+file_item +'.csv')
        body_model_info = pd.read_json(os.path.join(input_path, f'model_{body_model}_{participant}_{file_item}.json'), typ='series')
        fps_dancer = body_model_info['fps']
        
        # Energy file wholebody guitarist
        input_path = '/Users/atillajv/CODE/RITMO/SYNCHRONICITY/scripts/energy_toiviainen_adapted/output/28032025_guitarist/'
        participant = file_item.split('_')[2]
        print(input_path + 'Energy_wholebody_'+participant+'_'+file_item +'.csv', 'HELLOOO')
        g_df_energy_wholebody = pd.read_csv(input_path + 'Energy_wholebody_'+participant+'_'+file_item +'.csv')
        body_model_info = pd.read_json(os.path.join(input_path, f'model_{body_model}_{participant}_{file_item}.json'), typ='series')
        fps_guitar = body_model_info['fps']
        
        clap_time = df_clap_times[df_clap_times['filename'] == file_item]['peak_time'].iloc[0]
    ...
    #                print('Test5')
            ...
            # Fetching energy wholebody within interval

            p_list_energy_wholebody = p_df_energy_wholebody.loc[(p_df_energy_wholebody.iloc[:,0]/fps_dancer >= tI_0.iloc[i] - clap_time) & (p_df_energy_wholebody.iloc[:,0]/fps_dancer <= tI_1.iloc[i] - clap_time)]

            g_list_energy_wholebody = g_df_energy_wholebody.loc[(g_df_energy_wholebody.iloc[:,0]/fps_guitar >= tI_0.iloc[i] - clap_time) & (g_df_energy_wholebody.iloc[:,0]/fps_guitar <= tI_1.iloc[i] - clap_time)]
        ...
        # for i, item in enumerate(tI_0):
            ...
            df_store.loc[store_i] = {
                ...
                'g_KE_wholebody' : g_list_energy_wholebody['E_kin_sum'].mean(),
                'g_KE_wholebody_avg' : g_df_energy_wholebody['E_kin_sum'].mean(),
                'p_KE_wholebody' : p_list_energy_wholebody['E_kin_sum'].mean(),
                'p_KE_wholebody_avg' : p_df_energy_wholebody['E_kin_sum'].mean(),
                'g_PE_wholebody' : g_list_energy_wholebody['E_pot_sum'].mean(),
                'g_PE_wholebody_avg' : g_df_energy_wholebody['E_pot_sum'].mean(),
                'p_PE_wholebody' : p_list_energy_wholebody['E_pot_sum'].mean(),
                'p_PE_wholebody_avg' : p_df_energy_wholebody['E_pot_sum'].mean(),
                'p_RE_wholebody' : p_list_energy_wholebody['E_rot_sum'].mean(),
                'p_RE_wholebody_avg' : p_df_energy_wholebody['E_rot_sum'].mean(),
                'g_RE_wholebody' : g_list_energy_wholebody['E_rot_sum'].mean(),
                'g_RE_wholebody_avg' : g_df_energy_wholebody['E_rot_sum'].mean(),
                'p_TE_wholebody' : p_list_energy_wholebody['E_trans_sum'].mean(),
                'p_TE_wholebody_avg' : p_df_energy_wholebody['E_trans_sum'].mean(),
                'g_TE_wholebody' : g_list_energy_wholebody['E_trans_sum'].mean(),
                'g_TE_wholebody_avg' : g_df_energy_wholebody['E_trans_sum'].mean(),
                'annot_frac' : '',
                'number': i,
                }
            store_i = store_i+1
...
print(len(df_store), 'df_store before')
df_store.drop_duplicates(subset=None, keep="first", inplace=True)
print(len(df_store), 'df_store after')
