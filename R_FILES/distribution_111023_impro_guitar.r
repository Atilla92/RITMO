...
name_dance_mode <- df_filtered %>%
  select(Name, Music_Imp) %>%
  distinct()  # Get unique combinations of Name and Music_Imp
...
# Group by Music_Imp and calculate the averages and standard errors for specific columns
...
summary_stats_by_mode <- summary_table %>%
  ...
  select(Music_Imp, A1, A2, A3, Total_Avg) %>%
...
ggplot(summary_stats_by_mode, aes(x = Music_Imp, y = Value, fill = Cluster)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  geom_errorbar(aes(ymin = Value - SE, ymax = Value + SE),
                width = 0.3, position = position_dodge(0.7)) +
  labs(x = "Dance Condition", y = "Average (%)") +
  ggtitle("Average (%) Music Melody") +
  theme_minimal() +
  ...
  scale_x_discrete(
    name = "Music Imporov/Choreo",
    labels = c("D1" = "Impro", "D5" = "Mixed", "D6" = "Choreo")) +
  scale_fill_manual(values = custom_palette,
                    name = "Melodic Level",
                    labels = c("A1" = "Low - Acompanñamiento", "A2" = "Middle - Interludio ", "A3" = "High - Falseta"))
...
clustering_data <- na.omit(unique_data$Music_Imp)
print(unique_name_count_per_mode)

#pairwise.t.test(unique_data$Guitarra_Level, unique_data$Music_Imp, p.adjust.method = "bonferroni")
...
for (baile_level in unique_baile_levels) {
  ...
  condition_pairs <- combn(unique(data_baile_level$Music_Imp), 2, simplify = FALSE)
  for (pair in condition_pairs) {
    ...
    data_condition1 <- data_baile_level %>%
      filter(Music_Imp == condition1)
    data_condition2 <- data_baile_level %>%
      filter(Music_Imp == condition2)
...
t_test_results <- data.frame()
# Loop through unique Guitarra_Levels
unique_baile_levels <- unique(unique_data$Music_Imp)
for (baile_level in unique_baile_levels) {
  # Subset the data for the current Guitarra_Level
  data_baile_level <- unique_data %>%
    filter(Music_Imp == baile_level)
...
plot1<- ggplot(summary_stats_by_mode, aes(x = Music_Imp, y = Value, fill = Cluster)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  geom_errorbar(aes(ymin = Value - SE, ymax = Value + SE),
                width = 0.3, position = position_dodge(0.7)) +
  labs(x = "Dance Condition", y = "Average (%)") +
...
plot2<- ggplot(summary_stats_by_mode, aes(x = Music_Imp, y = Value, fill = Cluster)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  geom_errorbar(aes(ymin = Value - SE, ymax = Value + SE),
                width = 0.3, position = position_dodge(0.7)) +
  labs(x = "Dance Condition", y = "Average (%)") +
...
t_test_results <- data.frame()
# Loop through unique Guitarra_Levels
unique_baile_levels <- unique(df_filtered_R1$Music_Imp)
for (baile_level in unique_baile_levels) {
  # Subset the data for the current Guitarra_Level
  data_baile_level <- unique_data %>%
    filter(Music_Imp == baile_level)
...
summary_table_2 <- left_join(summary_table_2, distinct(df_filtered %>% select(Name, Music_Imp)))
summary_table_2 <- left_join(summary_table_2, distinct(df_filtered %>% select(Participant, Music_Imp)))
head(summary_table_2)

# ALl analysis
summary_table_2$Music_Imp <- as.factor(summary_table_2$Music_Imp)
summary_table_2$Palo <- as.factor(summary_table_2$Palo)
summary_table_2$Music_Imp <- factor(summary_table_2$Music_Imp, levels = c("D6", 'D5', 'D1'))
...
m03  = lmer(percentage_occurrence ~   Palo * Music_Imp + (1 | Participant)  , data = summary_table_2 )
m04  = lmer(percentage_occurrence ~   Music_Imp * Guitarra_Level + (1 | Participant)  , data = summary_table_2 )
m05  = lmer(percentage_occurrence ~   Music_Imp  + (1 | Participant)  , data = summary_table_2 )
...
m03  = lmer(Q3 ~   Music_Imp + (1 | Participant)  , data = summary_table_2)
